...
実行が必要ということは副作用についての考慮が必要、また、実行するには、バインディングされる変数の仮の値を仮定して実行する必要があります。
そのため、 Node.js で PostgreSQL のクエリから入力や出力のデータモデルのコードを生成するには色々考慮が必要な状況でした。

PostgreSQL のクライアントとサーバー間の通信で利用される メッセージベースのプロトコルの Describe を使えば解析されたクエリからメタデータを取得することができます。
